...
{
    [BurstCompile]
    [WriteGroup(typeof(URPMaterialPropertyBumpScale))]
    internal struct TweenURPBumpScale : IComponentData, ITweenId, ITweenInfo<float>
    {
        public uint Id;
        public float Start;
        public float End;

        public TweenURPBumpScale(in uint id, in float start, in float end)
        {
            Id = id;
            Start = start;
            End = end;
