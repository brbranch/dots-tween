...
{
    [BurstCompile]
    [WriteGroup(typeof(URPMaterialPropertyMetallic))]
    internal struct TweenURPMetallic : IComponentData, ITweenId, ITweenInfo<float>
    {
        public uint Id;
        public float Start;
        public float End;

        public TweenURPMetallic(in uint id, in float start, in float end)
        {
            Id = id;
            Start = start;
            End = end;
