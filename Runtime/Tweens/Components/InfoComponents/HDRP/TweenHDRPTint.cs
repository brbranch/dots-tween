...
{
    [BurstCompile]
    [WriteGroup(typeof(HDRPMaterialPropertyBaseColor))]
    internal struct TweenHDRPTint : IComponentData, ITweenId, ITweenInfo<float4>
    {
        public uint Id;
        public float4 Start;
        public float4 End;

        public TweenHDRPTint(in uint id, in float4 start, in float4 end)
        {
            Id = id;
            Start = start;
            End = end;
