...
{
    [BurstCompile]
    [WriteGroup(typeof(HDRPMaterialPropertyUnlitColor))]
    internal struct TweenHDRPTintUnlit : IComponentData, ITweenId, ITweenInfo<float4>
    {
        public uint Id;
        public float4 Start;
        public float4 End;

        public TweenHDRPTintUnlit(in uint id, in float4 start, in float4 end)
        {
            Id = id;
            Start = start;
            End = end;
