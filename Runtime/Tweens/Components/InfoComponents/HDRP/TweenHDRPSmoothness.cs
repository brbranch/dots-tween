...
{
    [BurstCompile]
    [WriteGroup(typeof(HDRPMaterialPropertySmoothness))]
    internal struct TweenHDRPSmoothness : IComponentData, ITweenId, ITweenInfo<float>
    {
        public uint Id;
        public float Start;
        public float End;

        public TweenHDRPSmoothness(in uint id, in float start, in float end)
        {
            Id = id;
            Start = start;
            End = end;
