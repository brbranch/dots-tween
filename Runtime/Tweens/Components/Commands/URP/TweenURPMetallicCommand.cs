...
{
    [BurstCompile]
    public struct TweenURPMetallicCommand : IComponentData, ITweenParams, ITweenInfo<float>
    {
        ...
        public TweenURPMetallicCommand(in float start, in float end, in float duration, TweenParams tweenParams = default)
        {
            ...
            Start = start;
            End = end;
            tweenParams.GenerateId(TypeManager.GetTypeIndex<TweenURPMetallic>().Value);
        }
