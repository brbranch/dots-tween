...
namespace DotsTween.Tweens
{
    ...
    public struct TweenURPSmoothnessCommand : IComponentData, ITweenParams, ITweenInfo<float>
    {
        ...
        public TweenURPSmoothnessCommand(in Entity entity, in float start, in float end, in float duration, TweenParams tweenParams = default)
        {
            tweenParams.Duration = duration;
            tweenParams.Id = tweenParams.GenerateId(entity.GetHashCode(), start.GetHashCode(), end.GetHashCode(), TypeManager.GetTypeIndex<TweenURPSmoothness>().Value);
