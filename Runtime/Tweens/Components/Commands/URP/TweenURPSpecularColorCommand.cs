...
namespace DotsTween.Tweens
{
    ...
    public struct TweenURPSpecularColorCommand : IComponentData, ITweenParams, ITweenInfo<float4>
    {
        ...
        public TweenURPSpecularColorCommand(in Entity entity, in float4 start, in float4 end, in float duration, TweenParams tweenParams = default)
        {
            tweenParams.Duration = duration;
            tweenParams.Id = tweenParams.GenerateId(entity.GetHashCode(), start.GetHashCode(), end.GetHashCode(), TypeManager.GetTypeIndex<TweenURPSpecularColor>().Value);
