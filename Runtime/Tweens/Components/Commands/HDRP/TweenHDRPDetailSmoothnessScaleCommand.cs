...
{
    [BurstCompile]
    public struct TweenHDRPDetailSmoothnessScaleCommand : IComponentData, ITweenParams, ITweenInfo<float>
    {
        ...
        public TweenHDRPDetailSmoothnessScaleCommand(in float start, in float end, in float duration, TweenParams tweenParams = default)
        {
            ...
            Start = start;
            End = end;
            tweenParams.GenerateId(TypeManager.GetTypeIndex<TweenHDRPDetailSmoothnessScale>().Value);
        }
