...
namespace DotsTween.Tweens
{
    ...
    public struct TweenHDRPSmoothnessRemapMaxCommand : IComponentData, ITweenParams, ITweenInfo<float>
    {
        ...
        public TweenHDRPSmoothnessRemapMaxCommand(in Entity entity, in float start, in float end, in float duration, TweenParams tweenParams = default)
        {
            tweenParams.Duration = duration;
            tweenParams.Id = tweenParams.GenerateId(entity.GetHashCode(), start.GetHashCode(), end.GetHashCode(), TypeManager.GetTypeIndex<TweenHDRPSmoothnessRemapMax>().Value);
